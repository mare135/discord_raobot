...
export default new ModalComponent({
  ...
  async execute(interaction: ModalSubmitInteraction) {
    const pointInputData = parseInt(
      interaction.fields.getTextInputValue('pointInput')
    );
    ...
    await interaction.reply(
      `${
        client.users.cache.get(userIdData)?.username || 'NO USER'
      }に${pointInputData}ポイントを付与しました。`
    );
  },
});
